...
const METRIC_CONFIG = {
    categorias: {
        ...
        description: "Distribuição dos ativos cadastrados por categoria de produto.",
        totalLabel: "Ativos no inventário",
        dataKey: "categorias",
    },
    status: {
        title: "Quantidade por status",
        description: "Mostra como os ativos estão distribuídos por situação operacional.",
        totalLabel: "Ativos analisados",
        dataKey: "status",
    },
    marcas: {
        title: "Quantidade por marca",
        description: "Ranking das marcas com mais ativos cadastrados no inventário.",
        totalLabel: "Ativos analisados",
        dataKey: "marcas",
    },
    locais: {
        title: "Quantidade por localização",
        description: "Distribuição dos ativos por local, setor ou ponto de armazenamento.",
        totalLabel: "Ativos analisados",
        dataKey: "locais",
    },
    evolucao: {
        title: "Evolução de cadastros",
        description: "Quantidade de ativos cadastrados por dia no período selecionado.",
...
async function loadDashboardProducts(showLoading = true) {
    ...
    try {
        const response = await fetch(`${DASHBOARD_PRODUCTS_ENDPOINT}?${params.toString()}`, {
            headers: {
        ...
        renderSummaryCards();
        renderCurrentChart();
        setStatus("Dados sincronizados.", formatLastUpdate(dashboardData.gerado_em));
    } catch (error) {
        console.error(error);
        dashboardData = DEFAULT_DASHBOARD_DATA;
        renderSummaryCards();
        renderCurrentChart();
        setStatus("Não foi possível carregar os dados.", "Confira a conexão com o banco e tente novamente.");
    } finally {
        loading = false;
    }
}
...
function normalizeDashboardPayload(payload) {
    const data = payload && typeof payload === "object" ? payload : DEFAULT_DASHBOARD_DATA;
    const resumo = data.resumo && typeof data.resumo === "object" ? data.resumo : DEFAULT_DASHBOARD_DATA.resumo;
    const categoriaSelecionada = data.categoria_selecionada && typeof data.categoria_selecionada === "object"
...
function populateCategoryFilter(categories) {
    ...
    categories.forEach((category) => {
        categoryFilter.appendChild(createOption(category.id, `${category.nome} (${formatNumber(category.total)})`));
    });

    categoryFilter.value = categories.some((category) => category.id === previousValue) ? previousValue : "todos";
...
function renderSummaryCards() {
    ...
    const selected = dashboardData.categoria_selecionada || DEFAULT_DASHBOARD_DATA.categoria_selecionada;
    ...
    setText("selectedTypeMetric", selected.id === "todos" ? "Todos" : selected.nome);

    const selectedDetail = selected.id === "todos"
    ...
    if (largest) {
        ...
        setText("largestTypeDetail", `${formatNumber(largest.total)} ativos, ${formatPercent(largest.percentual)} do total.`);
    } else {
...
function getCurrentRows(config) {
    const rows = Array.isArray(dashboardData[config.dataKey]) ? dashboardData[config.dataKey] : [];
...
function renderChart(rows, config) {
    ...
    const gridColor = styles.getPropertyValue("--line").trim() || "rgba(255,255,255,.13)";
            ...
            datasets: [
                {
                    ...
                    borderColor: isLine ? styles.getPropertyValue("--mint").trim() || "#66d5c2" : palette,
                ...
                tooltip: {
                    callbacks: {
                        label(context) {
                            ...
                            const total = calculateRowsTotal(chartRows);
...
function getSafeChartType(chartType, metric) {
    if (metric === "evolucao" && ["pie", "doughnut", "polarArea"].includes(chartType)) {
        return "line";
    }
...
function buildChartPalette(size) {
    ...
    return Array.from({ length: size }, (_item, index) => baseColors[index % baseColors.length]);
}
...
function renderRanking(rows, total) {
    ...
    if (!rows.length || total === 0) {
        container.innerHTML = '<div class="ranking-empty">Nenhum dado encontrado para o filtro atual.</div>';
        return;
    }
...
function renderTable(rows, total) {
    ...
    if (!rows.length || total === 0) {
        tableBody.innerHTML = '<tr><td colspan="3">Nenhum dado encontrado para o filtro atual.</td></tr>';
        return;
    }
